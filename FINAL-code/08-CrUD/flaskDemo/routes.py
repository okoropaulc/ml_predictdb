...
def home():
    ...
    results3 = Employee.query.join(Works_On,Employee.ssn == Works_On.essn) \
               .add_columns(Employee.fname, Employee.lname, Employee.minit, Employee.ssn, Works_On.pno, Works_On.essn) \
               .join(Project, Project.pnumber == Works_On.pno).add_columns(Employee.fname,Employee.lname,Employee.minit,Employee.ssn,Works_On.pno,Project.pname,Works_On.essn)
    return render_template('assign_home.html', title='Join',joined_m_n=results3)

#<h2><a class="article-title" href="{{ url_for('assign', pno=row.pno,essn=row.essn) }}">{{ row.pname }}</a></h2>
...
def new_assign():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('You have made new assignment!', 'success')
        return redirect(url_for('home'))
...
def update_assign(essn,pno):
    ...
    if form.validate_on_submit():   
        ...
        return redirect(url_for('assign', pno=assign.pno, essn=assign.essn))
    elif request.method == 'GET':             
