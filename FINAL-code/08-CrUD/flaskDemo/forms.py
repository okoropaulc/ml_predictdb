...
pno = Works_On.query.with_entities(Works_On.pno).distinct()

essn_results=list()
for row in essns:
    rowDict=row._asdict()
    essn_results.append(rowDict)
essnChoices = [(row['essn'],row['essn']) for row in essn_results]

pno_results=list()
for row in pno:
    rowDict=row._asdict()
    pno_results.append(rowDict)
pnoChoices = [(row['pno'],row['pno']) for row in pno_results]

class AssignUpdateForm(FlaskForm):
    essn = SelectField("Employee's SSN", choices=essnChoices)
    pno = SelectField("Project Number", choices=pnoChoices, coerce=int)
    hours=StringField('Work Hours:', validators=[DataRequired(),Length(max=15)])
    
    submit = SubmitField('Update this assignment')



essns1 = Employee.query.with_entities(Employee.ssn)
pnum1 = Project.query.with_entities(Project.pnumber)

essn_results1=list()
for row in essns1:
    rowDict=row._asdict()
    essn_results1.append(rowDict)
essnChoices1 = [(row['ssn'],row['ssn']) for row in essn_results1]

pno_results1=list()
for row in pnum1:
    rowDict=row._asdict()
    pno_results1.append(rowDict)
pnoChoices1 = [(row['pnumber'],row['pnumber']) for row in pno_results1]

class AssignForm(FlaskForm):
    essn = SelectField("Employee's SSN", choices=essnChoices1)
    pno = SelectField("Project Number", choices=pnoChoices1, coerce=int)
    hours = IntegerField("Employee Work Hours", validators=[DataRequired()])
    submit = SubmitField('Assign employee')

    def validate_essn(self, essn):
        essn_a = Works_On.query.filter_by(essn=essn.data).first()
        if essn_a:
            def validate_pno(self,pno):
                pno_a = Works_On.query.filter_by(pno=pno.data).first()
                raise ValidationError('That employee is working this project already.')
